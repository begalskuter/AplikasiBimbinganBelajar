...
const mockGurus = [
  { id: 1, name: 'Dewi Puspitasari, S.Pd', email: 'dewi@email.com', kota: 'Yogyakarta', mataPelajaran: ['Matematika SMP', 'Matematika SMA'], rating: 4.9, totalSiswa: 18, status: 'aktif', isVerified: true, tanggalBergabung: '2026-01-15' },
  { id: 2, name: 'Ahmad Ridwan, M.Pd', email: 'ahmad@email.com', kota: 'Semarang', mataPelajaran: ['Fisika SMA'], rating: 4.8, totalSiswa: 12, status: 'aktif', isVerified: true, tanggalBergabung: '2026-02-10' },
  { id: 3, name: 'Siti Nurhaliza, S.Si', email: 'siti@email.com', kota: 'Magelang', mataPelajaran: ['Kimia SMA', 'Kimia Olimpiade'], rating: 4.7, totalSiswa: 9, status: 'aktif', isVerified: true, tanggalBergabung: '2026-02-28' },
  { id: 4, name: 'Budi Kurniawan, S.Pd', email: 'budi.k@email.com', kota: 'Jakarta', mataPelajaran: ['B. Inggris SMP', 'B. Inggris SMA'], rating: 4.6, totalSiswa: 15, status: 'aktif', isVerified: true, tanggalBergabung: '2026-03-05' },
  { id: 5, name: 'Rina Putri, M.Pd', email: 'rina.p@email.com', kota: 'Bandung', mataPelajaran: ['B. Indonesia SMA'], rating: 4.5, totalSiswa: 7, status: 'nonaktif', isVerified: true, tanggalBergabung: '2026-03-18' },
  { id: 6, name: 'Hendra Wijaya, S.Pd', email: 'hendra@email.com', kota: 'Surabaya', mataPelajaran: ['Matematika SMP'], rating: 4.4, totalSiswa: 5, status: 'aktif', isVerified: true, tanggalBergabung: '2026-04-02' },
  { id: 7, name: 'Maya Anggraeni, M.Si', email: 'maya@email.com', kota: 'Solo', mataPelajaran: ['Biologi SMA'], rating: 0, totalSiswa: 0, status: 'aktif', isVerified: false, tanggalBergabung: '2026-05-19' },
  { id: 8, name: 'Fajar Setiawan, S.Pd', email: 'fajar@email.com', kota: 'Purwokerto', mataPelajaran: ['Sejarah SMA'], rating: 0, totalSiswa: 0, status: 'aktif', isVerified: false, tanggalBergabung: '2026-05-20' },
];

export default function DataGuru() {
  const [gurus, setGurus] = useState(mockGurus);
  ...
  const filtered = gurus.filter(g => {
    ...
    const matchFilter = filterStatus === 'all' ||
      (filterStatus === 'aktif' && g.status === 'aktif') ||
      (filterStatus === 'nonaktif' && g.status === 'nonaktif');
    return matchSearch && matchFilter;
  });

  const handleToggleStatus = (id) => {
    setGurus(gurus.map(g =>
      g.id === id ? { ...g, status: g.status === 'aktif' ? 'nonaktif' : 'aktif' } : g
    ));
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 22 }}>
        {[
          { label: 'Total Guru', value: gurus.length, accent: '#185FA5', icon: 'ti-users' },
          { label: 'Guru Aktif', value: gurus.filter(g => g.status === 'aktif').length, accent: '#16a34a', icon: 'ti-user-check' },
          { label: 'Nonaktif', value: gurus.filter(g => g.status === 'nonaktif').length, accent: '#dc2626', icon: 'ti-user-off' },
          { label: 'Rata-rata Rating', value: (gurus.filter(g => g.rating > 0).reduce((s, g) => s + g.rating, 0) / (gurus.filter(g => g.rating > 0).length || 1)).toFixed(1), accent: '#d97706', icon: 'ti-star' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input type="text" placeholder="Cari nama, email, atau kota..." value={searchQuery}
                  ...
                  >
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setSelectedGuru(g)} style={{
                          padding: '5px 10px', background: '#f0f7ff', color: '#185FA5',
                          border: '1px solid #bfdbfe', borderRadius: 6, fontSize: 11, fontWeight: 600,
                          cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif",
                        }}>Detail</button>
                        <button onClick={() => handleToggleStatus(g.id)} style={{
                          padding: '5px 10px',
                          ...
                          borderRadius: 6, fontSize: 11, fontWeight: 600,
                          cursor: 'pointer', fontFamily: "'Plus Jakarta Sans', sans-serif",
                        }}>{g.status === 'aktif' ? 'Nonaktifkan' : 'Aktifkan'}</button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {selectedGuru && (
        <div style={{
          ...
          <div onClick={(e) => e.stopPropagation()} style={{
            ...
            fontFamily: "'Plus Jakarta Sans', sans-serif", maxHeight: '85vh', overflowY: 'auto',
          }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              {[
                ...
                { label: 'Bergabung', value: new Date(selectedGuru.tanggalBergabung).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }), icon: 'ti-calendar' },
              ].map(info => (
                <div key={info.label} style={{
                  background: '#f8fafc', border: '1px solid #f1f5f9', borderRadius: 10, padding: '12px 16px',
                }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => { handleToggleStatus(selectedGuru.id); setSelectedGuru(null); }} style={{
                flex: 1, padding: '12px',
