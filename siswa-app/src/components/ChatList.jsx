  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#fff", borderRight: "1px solid #E2E8F0" }}>
      <div style={{ padding: "20px", borderBottom: "1px solid #E2E8F0", background: "#F8FAFC" }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0F172A", margin: 0 }}>Pesan</h2>
      </div>
        ...
        {loading ? (
          ...
          chats.map(chat => {
            ...
            return (
              ...
              >
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#E2E8F0", flexShrink: 0, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", color: "#64748B", fontWeight: "bold", fontSize: 16 }}>
                  {opp.avatar ? <img src={opp.avatar} alt="avatar" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : opp.name[0]}
                </div>
                <div style={{ flex: 1, overflow: "hidden" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                    <div style={{ fontWeight: isActive ? 800 : 700, fontSize: 15, color: "#0F172A", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{opp.name}</div>
                    <div style={{ fontSize: 12, color: opp.unread > 0 ? "#185FA5" : "#94A3B8", fontWeight: opp.unread > 0 ? 700 : 400 }}>{formatDate(chat.last_message_time)}</div>
                  </div>
                  <div style={{ fontSize: 13, color: opp.unread > 0 ? "#334155" : "#64748B", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: opp.unread > 0 ? 600 : 400 }}>
                    {chat.last_message || "Belum ada pesan"}
                  </div>
                </div>
                {opp.unread > 0 && (
                  <div style={{ width: 22, height: 22, background: "#185FA5", borderRadius: "50%", color: "#fff", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 12 }}>
                    {opp.unread}
                  </div>
                )}
              </div>
